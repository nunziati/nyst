...
def general_interpolate_signal(signal, target_frames, interpolation_order):
...
def preprocess_interpolation(data, frames=150, order=2):
    ...
    for column in ['left_position X', 'left_position Y', 'right_position X', 'right_position Y',
        ...
        data[column] = data[column].apply(lambda x: general_interpolate_signal(json.loads(x), frames, 2) if len(json.loads(x)) != frames else x)
    
    return data   # Return the DataFrame with the interpolated signals

# Function to interpolate a single signal using cubic spline interpolation
def cubic_interpolate_signal(signal, target_frames):
    # Create an array of evenly spaced values between 0 and 1 for the original signal
    x_original = np.linspace(0, 1, len(signal))  
...
def cubic_interpolation(data, frames=150):
    '''
    ...
    for column in ['left_position X', 'left_position Y', 'right_position X', 'right_position Y',
        ...
        # Apply the interpolation function to each signal in the column
        data[column] = data[column].apply(lambda x: cubic_interpolate_signal(json.loads(x)) if len(json.loads(x)) != frames else x)
