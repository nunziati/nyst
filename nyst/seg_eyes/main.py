...
all_images = sorted(glob(os.path.join(DATA_DIR, "images_target_png/*")))
all_masks = sorted(glob(os.path.join(DATA_DIR, "Binary_Masks_new/*")))
# Subset if necessary
if SUBSET:
...
if CALCULATE_CLASS_WEIGHTS or not os.path.exists('class_weights.pkl'):
    ...
    class_weights = read_masks_and_compute_weights(os.path.join(DATA_DIR, "Binary_Masks_new/"))
